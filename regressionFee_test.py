# DEBIT transaction and fee is charged on debtor
# DEBIT transaction and fee is charged on creditor
# DEBIT transaction and fee is charged on default (payload without the field, default = debtor)
# CREDIT transaction and fee is charged on debtor
# CREDIT transaction and fee is charged on creditor (crymbo case)
# CREDIT transaction and fee is charged on default (payload without the field, default = debtor)
...
logging.basicConfig(format='%(asctime)s - %(levelname)s - %(message)s',
                    ...
                    filename="regression_fee.log")  # Name of log file
...
def main_flow():
    ...
    with requests.Session() as s:
        ...
        # Create Debit transaction and fee charged on Debtor
        ...
        paymentInit_payload = {
            ...
            "amount": amount,
            "fee": fee,
            "trx_fee_charged_on": beneficiary
        }
        try:
            ...
            if (paymentInit_response.status_code in (200, 201, 202, 204)) and (paymentId is not None):
                ...
                # settle Debit transaction (complete transaction)
        ...
        # Create Debit transaction and fee charged on Creditor
        ...
        # Create Credit transaction and fee charged on Debtor
        amount = random.uniform(1, 10)
        fee = random.uniform(0.5, 2)
        beneficiary = "DEBTOR"
        direction = "CREDIT"
        ...
        try:
            ...
            if (paymentInit_response.status_code in (200, 201, 202, 204)) and (paymentId is not None):
                ...
                # settle Credit transaction (complete transaction)
                paymentSettle_payload = {
                    "paymentId": paymentId
                }
                try:
        ...
        # Create Credit transaction and fee charged on Creditor
        amount = random.uniform(1, 10)
        fee = random.uniform(0.5, 2)
        beneficiary = "CREDITOR"
        direction = "CREDIT"
...
main_flow()
            ...
            # def decode_txn_id(string: str) -> str:
            #
