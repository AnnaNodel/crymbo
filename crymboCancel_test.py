...
logger = logging.getLogger("Crymbo_INX")
...
def main_flow_cancel():
    ...
    with requests.Session() as s:
        ...
        # randomly generating amount and fee
        ...
        try:
            ...
            if (paymentInit_response.status_code in (200, 201, 202, 204)) and (paymentId is not None):
                ...
                try:
                    paymentCancel_response = s.delete(paymentCancel_API + str(paymentId),
                                                      headers={'Authorization': "Bearer " + crymbo_token}, verify=False)
                    payment_status = paymentCancel_response.json().get("payment_status")

                    if paymentCancel_response.status_code in (200, 201, 202, 204):
                        logger.info("DEBIT Transaction is canceled!")
                        logger.info(paymentId)
                        logger.info(payment_status)
                    else:
                        logger.info(paymentCancel_response.status_code)
                except requests.exceptions.RequestException as e:
                    logger.exception(e)
        ...
        try:
            ...
            if (paymentInit_response.status_code in (200, 201, 202, 204)) and (paymentId is not None):
                ...
                try:
                    ...
                    if paymentCancel_response.status_code in (200, 201, 202, 204):
                        logger.info("CREDIT Transaction is canceled!")
                        logger.info(paymentId)
                        logger.info(payment_status)
                    else:
                        logger.info(paymentCancel_response.status_code)
